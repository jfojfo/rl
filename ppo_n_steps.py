...
config = {
    ...
    'model': 'pg.episode.ppo.cnn3d.256_steps',
    'model_dir': 'models',
...
class EpisodeCollector:
    ...
    def add(self, state, action, reward, done, *extra_pt_tensors):
        ...
        if len(self.actions) == cfg.collect_steps:
...
def train_(load_from):
    ...
    while not early_stop and epoch < cfg.max_epoch:
        ...
        env_1_rewards.append(reward[0])
        if done[0]:            
            print(f'Env 1: steps {len(env_1_rewards)}, reward {sum(env_1_rewards)}')
            writer.add_scalar('Reward/env_1_reward', sum(env_1_rewards), epoch)
            env_1_rewards = []
